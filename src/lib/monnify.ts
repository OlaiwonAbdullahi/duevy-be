...
/**
 * Read a Create Invoice's status — separate endpoint from getTransactionStatus,
 * since an invoiceReference isn't a transactionReference (they're generated by
 * different Monnify products). Used for due/vote payments, which now always go
 * through createInvoice(); getTransactionStatus is still used for the card-save
 * verification charge, which is a plain init-transaction, not an invoice.
 * invoiceStatus values beyond PENDING/PAID aren't fully confirmed — anything
 * else maps to PENDING (not FAILED) so an ambiguous state doesn't wrongly
 * fail a payment that might still resolve; the 48h give-up in the
 * reconciliation job is the real backstop for truly abandoned invoices.
 */
export async function getInvoiceStatus(invoiceReference: string): Promise<MonnifyTxnStatus | null> {
  const token = await getAccessToken();
  const url = `${env.MONNIFY_BASE_URL}/api/v1/invoice/${encodeURIComponent(invoiceReference)}/details`;
  const res = await fetch(url, {
    headers: { Authorization: `Bearer ${token}` },
  });
  const json = (await res.json()) as {
    requestSuccessful: boolean;
    responseBody?: { invoiceReference: string; invoiceStatus: string; amount: number };
    responseMessage?: string;
  };
  if (!res.ok || !json.requestSuccessful || !json.responseBody) {
    console.error(`[monnify] invoice-status lookup failed for ref=${invoiceReference}:`, JSON.stringify(json));
    return null;
  }

  const raw = json.responseBody.invoiceStatus;
  const paymentStatus = raw === 'PAID' ? 'PAID' : raw === 'EXPIRED' || raw === 'CANCELLED' ? 'FAILED' : 'PENDING';
  if (paymentStatus !== 'PAID') {
    console.log(`[monnify] invoice-status ref=${invoiceReference}:`, JSON.stringify(json.responseBody));
  }
  return {
    paymentStatus,
    amountPaid: json.responseBody.amount,
    paymentReference: json.responseBody.invoiceReference,
    transactionReference: json.responseBody.invoiceReference,
  };
}
