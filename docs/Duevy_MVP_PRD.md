...
>
> **Consequence 2:** a 300-student space collecting ₦5,000 each hits the ₦300,000 balance ceiling around student 60. Duevy nudges withdrawal at 70% of the ceiling and hard-warns at 90%: "Withdraw now — your space is close to its limit."
>
> **This is escapable, and cheaply.** TIER_3 (a government ID, ₦200, manual review) is **unlimited**. The upgrade is built — `POST /payout/kyc/upgrade` — and a rep keeps collecting on TIER_2 while it is reviewed. A space that outgrows the ceiling pays ₦200 once and stops having one, so the warning should offer the upgrade rather than only urging a withdrawal.
>
> ~~Reps who repeatedly hit the ceiling are the trigger to add a Tier 2 upgrade path (deferred, see §12).~~ **No longer deferred — the TIER_3 upgrade is built.** Note the tier naming: the API level is `TIER_3`, not "Tier 2".
...
| 3 | ~~**Balance ceiling**~~ — **ANSWERED, AND NO LONGER A PRODUCT CONSTRAINT.** Duevy's settlement account has no limit, so the ₦50,000 single-deposit cap never touches checkout and a due may be any amount (§3.4's Consequence 1 is void). The rep's own account is still capped at ₦300,000 on TIER_2 — but **TIER_3 is unlimited**, and the upgrade path is built (`POST /payout/kyc/upgrade`, ₦200, manual review). So the ceiling is now a ₦200 problem rather than a design constraint: warn at 70/90% on TIER_2, offer the upgrade, and it disappears. This retires the deferral in §12. |
